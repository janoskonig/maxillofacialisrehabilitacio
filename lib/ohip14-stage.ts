...
import { LEGACY_PATIENT_STAGE_TO_CODE } from './legacy-patient-stage-map';
...
export async function getCurrentEpisodeAndStage(
  ...
  if (hasStageEvents.rows.length > 0) {
    ...
    return { episodeId: null, stageCode: null, stage: null, useNewModel: true, deliveryDate: null };
  }

  const legacy = await pool.query(`SELECT episode_id, stage FROM patient_current_stage WHERE patient_id = $1`, [
    patientId,
  ]);
  ...
  const stageCode = LEGACY_PATIENT_STAGE_TO_CODE[stage] ?? 'STAGE_0';
  const deliveryDate =
    stage === 'fogpotlas_kesz' || stage === 'gondozas_alatt' ? await getDeliveryDateLegacy(pool, patientId) : null;
...
export async function getCurrentStageCodeForOhip(
  ...
  const legacy = await pool.query(`SELECT stage FROM patient_current_stage WHERE patient_id = $1`, [patientId]);
  const legacyStage = legacy.rows[0]?.stage as string | undefined;
  if (!legacyStage) return null;
  return LEGACY_PATIENT_STAGE_TO_CODE[legacyStage] ?? 'STAGE_0';
}
 ...
 */
async function getDeliveryDateLegacy(pool: Pool, patientId: string): Promise<Date | null> {
  try {
    const row = await pool.query(`SELECT stage_date FROM patient_current_stage WHERE patient_id = $1`, [patientId]);
