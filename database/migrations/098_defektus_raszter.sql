...
-- 098: Raszterezett defektus-osztályozás (maxilla / mandibula).
--
-- A Brown- (maxilla) és a Kovács–Dobák- (mandibula) osztályok helyett a
-- defektus KITERJEDÉSÉT jelöljük: egy-egy sematikus állcsontkép 6×8-as rácsán a
-- klinikus bejelöli az érintett mezőket (lib/defect-raster.ts,
-- components/patient-form/DefectRaster.tsx). A tárolt érték a bejelölt mezők
-- kulcsainak JSONB tömbje, pl. ["r2c1","r2c2"] — a sorok elölről hátrafelé
-- (metszők → tuber / ramus), az oszlopok a beteg jobb oldalától a bal oldaláig
-- (J4…J1 | B1…B4) haladnak. Üres tömb = nincs bejelölt mező.
--
-- A régi brown_fuggoleges_osztaly / brown_vizszintes_komponens /
-- kovacs_dobak_osztaly oszlopok archívként megmaradnak (az űrlapon már nem
-- szerkeszthetők, csak a korábban rögzített érték látszik).
--
-- A patients_full VIEW-t bővítjük (az új oszlopok a lista VÉGÉRE kerülnek — a
-- CREATE OR REPLACE VIEW csak így engedi), és az INSTEAD OF INSERT/UPDATE
-- trigger függvényeket is kiegészítjük, hogy a view-n keresztüli írás ne dobja
-- el csendben az adatot. A view többi része szó szerint a 076-os definíció.

ALTER TABLE patient_anamnesis
  ADD COLUMN IF NOT EXISTS maxilla_defektus_raszter JSONB NOT NULL DEFAULT '[]'::jsonb,
  ADD COLUMN IF NOT EXISTS mandibula_defektus_raszter JSONB NOT NULL DEFAULT '[]'::jsonb;

COMMENT ON COLUMN patient_anamnesis.maxilla_defektus_raszter IS
  'Maxilladefektus kiterjedése a 6×8-as állcsont-raszteren: a bejelölt mezők kulcsai (["r2c1",…], lib/defect-raster.ts). Üres tömb = nincs jelölés.';
COMMENT ON COLUMN patient_anamnesis.mandibula_defektus_raszter IS
  'Mandibuladefektus kiterjedése a 6×8-as állcsont-raszteren: a bejelölt mezők kulcsai (["r3c0",…], lib/defect-raster.ts). Üres tömb = nincs jelölés.';
...
SELECT
  ...
  -- defektus-raszter (098) — ÚJ oszlopok, kötelezően a lista végén
  a.maxilla_defektus_raszter, a.mandibula_defektus_raszter
FROM patients p
...
-- Az INSTEAD OF INSERT/UPDATE triggerek anamnesis ága kiegészítve a két
-- raszter-oszloppal; a többi ág szó szerint a 076-os definíció.
...
BEGIN
  ...
  UPDATE patient_anamnesis SET
    ...
    maxilla_defektus_raszter = COALESCE(NEW.maxilla_defektus_raszter, '[]'::jsonb),
    mandibula_defektus_raszter = COALESCE(NEW.mandibula_defektus_raszter, '[]'::jsonb)
  WHERE patient_id = OLD.id;
