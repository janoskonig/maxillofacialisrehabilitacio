 ...
 * SQL CHECK constraint asserted by migration 026 MUST stay in sync. If they
 ...
 *   - Add the new value to APPOINTMENT_STATUS_VALUES, AND to migration 026,
 *     AND update lib/active-appointment.ts to decide whether the new value
 *     is "active" / "visible" / "cancelled".
 *   - OR drop the new value from migration 026 if it shouldn't be there.
 *
...
describe('Appointment status taxonomy — TS ↔ SQL parity', () => {
  it('the canonical TS list contains exactly 4 non-null values', () => {
  ...
  it('migration 026 lists the same 4 values inside the canonical CHECK constraint', () => {
    const path = join(
      __dirname,
      '..',
      '..',
    ...
    // The canonical block is the ALTER TABLE ... ADD CONSTRAINT
    // appointments_appointment_status_check ... CHECK (...).
    const checkBodyMatch = sql.match(
      /ADD CONSTRAINT appointments_appointment_status_check[\s\S]*?CHECK \(([\s\S]*?)\);/
    );
    expect(checkBodyMatch, 'canonical CHECK block missing from migration 026').toBeTruthy();
    const checkBody = checkBodyMatch![1];

    for (const value of APPOINTMENT_STATUS_VALUES) {
      expect(
        checkBody.includes(`'${value}'`),
        `migration 026 CHECK is missing canonical value: ${value}`
      ).toBe(true);
    }
    ...
    seen.forEach((lit) => {
      expect(
        allowed.has(lit),
        `migration 026 CHECK references an unknown appointment_status literal: ${lit}`
      ).toBe(true);
    });
  });

  it('migration 026 explicitly accepts NULL (no reliance on SQL 3-valued-logic loophole)', () => {
    const path = join(
      __dirname,
      '..',
      '..',
    ...
    expect(checkBodyMatch).toBeTruthy();
    const checkBody = checkBodyMatch![1];
    ...
    expect(
      ...
      'migration 026 CHECK must explicitly accept NULL via `appointment_status IS NULL`'
    ).toBe(true);
    ...
    expect(
      ...
      'migration 026 CHECK must NOT require NOT NULL (would break the pending semantic)'
    ).toBe(false);
  });
  ...
  it('compile-time exhaustiveness: every canonical value must be reachable through AppointmentStatus', () => {
    ...
    function classify(s: AppointmentStatus): 'active' | 'cancelled' {
      switch (s) {
        case 'completed':
        case 'no_show':
          return 'active';
