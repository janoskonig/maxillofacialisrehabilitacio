    ...
    <div className="mb-6 rounded-lg border border-slate-200 bg-slate-50/80 p-4">
          ...
          <table className="w-full text-[11px] border-collapse">
            ...
            <tbody>
              {OHIP14_IMPACT_BANDS.map((b) => (
                <tr key={b.level} className="border-b border-slate-100 text-slate-700">
                  <td className="py-1 pr-2 whitespace-nowrap font-mono">
                    {b.min}–{b.max}
                  </td>
                  <td className="py-1 pr-2 whitespace-nowrap">{b.label}</td>
                  <td className="py-1 text-slate-600">{b.description}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {hasScore && band && (
        <div className="mt-3 pt-3 border-t border-slate-200 space-y-3">
          ...
          <p className="text-xs text-slate-700">{band.description}</p>
            ...
            {dimensions.map((d) => (
              ...
              >
                ...
                <span className="text-slate-900 font-medium shrink-0">
                  {d.score}/8 <span className="text-slate-500 font-normal">({d.impact})</span>
                </span>
              </div>
            ))}
          </div>
