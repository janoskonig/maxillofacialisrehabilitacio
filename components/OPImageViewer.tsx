...
export function OPImageViewer({ patientId, patientName, isOpen, onClose }: OPImageViewerProps) {
  ...
  const loadOPDocuments = async () => {
    ...
    try {
      setLoading(true);
      const response = await fetch(`/api/patients/${patientId}/documents/op`);
      if (!response.ok) {
        throw new Error('Failed to load OP documents');
      }
      const data = await response.json();
      setDocuments(data.documents || []);
    } catch (error) {
      console.error('Error loading OP documents:', error);
      setDocuments([]);
    } finally {
      setLoading(false);
    }
          ...
          <div className="flex items-center justify-between">
            <div className="flex-1">
              <h2 className="text-lg font-semibold">
                {patientName ? `${patientName} - OP dokumentumok` : 'OP dokumentumok'}
              </h2>
              {currentDocument && (
