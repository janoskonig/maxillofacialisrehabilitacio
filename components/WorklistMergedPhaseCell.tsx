...
function SiteChips({
  ...
  return (
    <span className="inline-flex flex-wrap gap-1 items-center align-middle">
      {toothNumber != null && (
        <span className="text-[10px] font-medium px-1.5 py-0.5 rounded bg-teal-100 dark:bg-teal-950/50 text-teal-800 dark:text-teal-300 border border-teal-200/80">
          Fog {toothNumber}
        </span>
      )}
      {jaw != null && (
        <span className="text-[10px] font-medium px-1.5 py-0.5 rounded bg-sky-100 dark:bg-sky-950/50 text-sky-800 dark:text-sky-300 border border-sky-200/80">
          {jawLabelHu(jaw)}
        </span>
      )}
    </span>
    ...
    <div className="flex flex-col gap-1 min-w-0">
      <div className="flex flex-wrap items-baseline gap-x-2 gap-y-1">
        ...
        {item.mergedWorkPhase && (
          <span className="text-[10px] font-semibold uppercase tracking-wide shrink-0 px-1.5 py-0.5 rounded bg-violet-100 dark:bg-violet-950/50 text-violet-800 dark:text-violet-300 border border-violet-200 dark:border-violet-800">
      ...
      {item.mergedWorkPhase && item.mergedWorkPhaseParts && item.mergedWorkPhaseParts.length > 0 && (
        <div className="text-xs text-gray-600 dark:text-gray-400 border-l-2 border-violet-200 dark:border-violet-800 pl-2 space-y-1">
          <span className="font-medium text-gray-700 dark:text-gray-300">Összetétel:</span>
