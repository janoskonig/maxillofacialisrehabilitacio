...
function EventRow({
  ...
  return (
    <li className="relative flex gap-3 pb-4 border-l-2 border-gray-100 pl-4 ml-2 last:pb-0">
      <div className="absolute -left-[9px] top-1 h-4 w-4 rounded-full bg-white border-2 border-medical-primary/40 flex items-center justify-center">
      ...
      <div className="flex-1 min-w-0">
                ...
                {event.type === 'stage_change' &&
                  ...
                  canPatchStageEventId(event.id) && (
                    <button
                      ...
                      className="p-0.5 text-gray-400 hover:text-medical-primary"
        ...
        {event.type === 'stage_change' && (
          <>
            ...
            {event.payload.note && (
              <p className="text-sm text-gray-600 mt-1">{event.payload.note}</p>
            )}
            {event.payload.authorDisplay && (
              <p className="text-xs text-gray-400 mt-0.5">{event.payload.authorDisplay}</p>
            )}
          </>
        )}
        ...
        {event.type === 'consilium' && (
          <div>
            ...
            {event.payload.verdictSummary && (
              <p className="text-sm text-gray-700 mt-1">{event.payload.verdictSummary}</p>
            )}
          </div>
        )}
        ...
        {event.type === 'consilium_prep' && (
          <div>
            ...
            <p className="text-sm text-gray-800 mt-1 whitespace-pre-wrap">{event.payload.body}</p>
            {event.payload.authorDisplay && (
              <p className="text-xs text-gray-400 mt-0.5">— {event.payload.authorDisplay}</p>
            )}
          </div>
        ...
        {event.type === 'consilium_prep_link' && (
          <div>
            <p className="text-sm text-gray-800">Prep link megosztva</p>
            {event.payload.senderName && (
              <p className="text-xs text-gray-500">{event.payload.senderName}</p>
            )}
            <Link
        ...
        {event.type === 'milestone' && (
          <div>
            <p className="text-sm font-medium text-gray-900">{event.payload.label}</p>
            {event.payload.note && (
              <p className="text-sm text-gray-600 mt-0.5">{event.payload.note}</p>
            )}
          </div>
        )}
...
export function PatientCareTimeline({
  ...
  if (!data) {
    return (
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <p className="text-gray-500">Nincs timeline adat.</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-4 sm:p-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
        <h3 className="text-lg font-semibold text-gray-900">Páciens timeline</h3>
        <button
          type="button"
      ...
      {current && (
        <div className="mb-6 p-4 bg-medical-primary/10 border border-medical-primary/20 rounded-lg">
          ...
          {current.note && <p className="text-sm text-gray-700 mt-2">{current.note}</p>}
        </div>
      )}

      {filteredEpisodes.length === 0 ||
      filteredEpisodes.every((ep) => ep.events.length === 0) ? (
        <p className="text-gray-500 text-sm">Nincs megjeleníthető esemény a kiválasztott szűrővel.</p>
      ) : (
        <div className="space-y-4">
          <h4 className="text-base font-medium text-gray-900">Epizódok</h4>
          {filteredEpisodes.map((ep) => {
            ...
            return (
              <div key={ep.episodeId} className="border border-gray-200 rounded-lg overflow-hidden">
                <button
                  type="button"
                  ...
                  <div>
                    <span
                      className={`text-xs font-medium mr-2 ${
                        ep.status === 'open' ? 'text-green-600' : 'text-gray-500'
                      }`}
                    >
