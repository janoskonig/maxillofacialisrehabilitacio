...
export function Sidebar({ user, taskSummary, inboxSummary, onLogout, onFeedback }: SidebarProps) {
  ...
  return (
    <aside className="hidden md:flex md:flex-col md:w-60 md:flex-shrink-0 sticky top-0 h-screen bg-white border-r border-gray-200">
      ...
      <div className="border-t border-gray-200 px-2 py-3 space-y-0.5">
        ...
        <button
          type="button"
          onClick={onLogout}
          className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-red-700 hover:bg-red-50 transition-colors"
        >
          <LogOut className="w-5 h-5 flex-shrink-0" />
          <span className="flex-1 min-w-0 truncate text-left">Kijelentkezés</span>
        </button>
        <div className="px-3 pt-2 text-xs text-gray-400 truncate" title={user.email}>
