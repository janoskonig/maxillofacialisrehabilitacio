...
export default function TodaysAppointmentsPage() {
  ...
  const doctors = useMemo(() => {
    ...
    for (const a of appointments) {
      if (a.dentistEmail)
        map.set(a.dentistEmail, a.dentistName || a.dentistEmail);
    }
    return Array.from(map, ([email, name]) => ({ email, name })).sort((x, y) =>
  ...
  const visible = useMemo(
    () =>
      selectedDentist
        ? appointments.filter((a) => a.dentistEmail === selectedDentist)
        : appointments,
  ...
  const noShow = visible.filter(
    (a) => a.appointmentStatus === "no_show",
  ).length;
    ...
    >
      <div className="space-y-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <p className="text-body-sm text-gray-500 dark:text-gray-400 capitalize">
            {today}
          </p>
          {doctors.length > 1 && (
              ...
              >
                ...
                {doctors.map((d) => (
                  <option key={d.email} value={d.email}>
                    {d.name} (
                    {
                      appointments.filter((a) => a.dentistEmail === d.email)
                        .length
                    }
                    )
                  </option>
        ...
        <Dashboard userRole={role ?? undefined} showAppointments={false} />
        {role === "beutalo_orvos" && <ReferredPatientsPanel />}

        <section aria-label="Mai időpontok" className="space-y-4">
          <h2 className="text-heading-3">Mai időpontok</h2>
          <RemoteDataState
            status={dataStatus}
            label="A mai időpontok"
            onRetry={fetchData}
          >
            {/* KPI-k */}
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              <StatCard
                ...
                icon={Clock}
                tone="warning"
                delta={pending > 0 ? "kimenetel rögzítendő" : undefined}
              />
              <StatCard
            ...
            {total === 0 ? (
              <EmptyState
                ...
                title={
                  selectedDentist
                    ? "Nincs időpont ehhez az orvoshoz"
                    : "Nincsenek mai időpontok"
                }
                description={
                  selectedDentist
