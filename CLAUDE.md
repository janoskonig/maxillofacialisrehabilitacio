...
- **Account deactivation**: `verifyAuth` also checks `users.active` in the DB (30s per-process cache in `lib/user-active-check.ts`), so a deactivated user's still-valid JWT stops working. `users.deactivated_at` (migration 100) separates admin-deactivated accounts from never-approved registrations (`active=false` + NULL); the guards (self, last active admin) and side effects live in `lib/user-deactivation.ts`.
...
- **Conditional offers** (`feltételes időpont`): `approval_status='pending'` rows with an `approval_token`; the patient approves/rejects via `/api/appointments/approve|reject` email links. `POST /api/appointments/[id]/cancel-and-offer` (staff `Lemondás` dialog, `components/CancelAppointmentDialog.tsx`) soft-cancels a booking and creates such an offer that inherits the episode/work-phase context and takes over the `episode_work_phases` link; final rejection releases it (`lib/conditional-offer-release.ts`).
